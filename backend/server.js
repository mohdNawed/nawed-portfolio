...
// Connect to DB for Serverless Functions
const connectMongo = async () => {
  if (db) return; // Use existing connection if Vercel keeps the function warm
...
// Middleware to ensure DB connection before every API request
...
app.post('/api/hire', async (req, res) => {
  ...
  if (!validateEmail(email)) return res.status(400).json({ success: false, message: 'Invalid format.' });
  ...
  try {
    ...
    const inquiryDocument = {
      ...
      budget: sanitizedBudget || 'Not specified',
      details: sanitizedDetails || 'Not provided',
      createdAt: new Date(), source: 'hire-form',
    };
    ...
    await transporter.sendMail({
      ...
      subject: `Hire Inquiry from ${sanitizedName}`,
      text: `New inquiry from ${sanitizedName}. Project: ${sanitizedProjectType}. Details: ${sanitizedDetails}`
    });
...
app.post('/api/contact', async (req, res) => {
  ...
  if (!name || !email || !message) return res.status(400).json({ success: false, message: 'Fields required.' });
  if (!validateEmail(email)) return res.status(400).json({ success: false, message: 'Invalid format.' });
  ...
  try {
    ...
    const messageDocument = {
      ...
      createdAt: new Date(), source: 'contact-form',
    };
...
// CRITICAL: Export the app instead of listening on a port!
module.exports = app;